...
void PopulateCommonParams(TfLiteContext* context,
  ...
  MicroPrintf("quant_params: %d %d", data.reference_op_data.output_multiplier,
              data.reference_op_data.output_shift);
  MicroPrintf("batch: %d", data.batches);
  MicroPrintf("accum_depth: %d", data.accum_depth);
  MicroPrintf("output_depth: %d", data.output_depth);
...
TfLiteStatus EvalQuantizedInt8(TfLiteContext* context, TfLiteNode* node,
  ...
  MicroPrintf("input_dims: %d %d %d %d", input_dims.n, input_dims.h,
              input_dims.w, input_dims.c);
  MicroPrintf("filter_dims: %d %d %d %d", filter_dims.n, filter_dims.h,
              filter_dims.w, filter_dims.c);
  MicroPrintf("bias_dims: %d %d %d %d", bias_dims.n, bias_dims.h, bias_dims.w,
              bias_dims.c);
  MicroPrintf("output_dims: %d %d %d %d", output_dims.n, output_dims.h,
              output_dims.w, output_dims.c);
  ...
  if (output_dim_count > 2 && data.accum_depth % 4 == 0) {
    ...
    MicroPrintf("input_offset: %d", fc_params.input_offset);
    MicroPrintf("output_offset: %d", fc_params.output_offset);
    MicroPrintf("filter_offset: %d", fc_params.filter_offset);
    MicroPrintf("activation.min: %d", fc_params.activation.min);
    MicroPrintf("activation.max: %d", fc_params.activation.max);
    ...
    for (int i = 0; i < output_dims.n * output_dims.c; i++) {
